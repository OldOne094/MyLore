...
describe("SearchPage", () => {
  ...
  it("searches the FTS backend for the URL query and lists rows", async () => {
    ...
    expect(invoke).toHaveBeenCalledWith("media_search", { query: "steins", contentType: null });
  ...
  it("shows a no-results state when the backend returns nothing", async () => {
    ...
    expect(invoke).toHaveBeenCalledWith("media_search", { query: "zzz", contentType: null });
  });
