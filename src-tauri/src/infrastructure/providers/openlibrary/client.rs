//! OpenLibrary REST transport (MISSION-057).
//!
...
//! maps transport/HTTP/JSON outcomes to `ProviderError`. OpenLibrary needs no
//! auth and accepts a small request rate (1 req/s default, 3 req/s with a UA
//! that identifies the app and a contact — we always send `APP_USER_AGENT`).
...
/// A polite, identifiable User-Agent (OpenLibrary rewards contact in the UA).
pub const APP_USER_AGENT: &str = concat!(
    "MyLore/",
    env!("CARGO_PKG_VERSION"),
    " (local-first media tracker; OpenLibrary credit)"
);

/// A reqwest-backed OpenLibrary client. `Clone` is cheap (one connection pool).
#[derive(Clone)]
...
impl OpenLibraryClient {
    ...
    pub async fn get<T: DeserializeOwned>(
        ...
        serde_json::from_str(&text).map_err(|e| ProviderError::InvalidResponse {
            provider: PROVIDER_ID.to_string(),
            message: format!("invalid OpenLibrary payload: {e}"),
        })
    }
}
...
mod tests {
    ...
    fn client_with(server: &MockServer) -> OpenLibraryClient {
        let http = reqwest::Client::builder()
            ...
            .build()
            .unwrap();
        OpenLibraryClient::with_endpoint(http, server.uri())
    }

    #[tokio::test]
    async fn get_sends_ua_and_parses() {
        ...
        let out: super::super::response::SearchResponse = client_with(&server)
            .get("/search.json", &[("q", "dune"), ("limit", "20")])
            .await
            .unwrap();
    ...
    async fn maps_429_to_rate_limited() {
        ...
        let err = client_with(&server)
            .get::<serde_json::Value>("/search.json", &[])
            .await
            .unwrap_err();
        assert!(matches!(
            err,
    ...
    async fn maps_404_to_not_found() {
        ...
        let err = client_with(&server)
            .get::<serde_json::Value>("/works/OLX.json", &[])
            .await
            .unwrap_err();
