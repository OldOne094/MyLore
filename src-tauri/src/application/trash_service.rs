...
impl TrashService {
    ...
    /// List active trash entries (media kind) for the trash page.
...
mod tests {
    ...
    async fn restore_rejects_unknown_or_non_media() {
        ...
        trash::insert(&pool, "t-x", "merge", "{}", "2026-01-01T00:00:00Z")
            .await
            .expect("insert");
        let err = service.restore_media("t-x").await.expect_err("merge kind");
        assert!(matches!(err, AppError::Validation(_)));
    }
